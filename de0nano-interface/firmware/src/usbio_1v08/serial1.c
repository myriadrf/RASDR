 ...
 **/
void sio1_init( WORD baud_rate ) __critical { // baud_rate max should be 57600 since int=2 bytes
/*	
...
void _transchar_s1(char c)
{
 while ( !TI1 ); // wait for TI=1 
 TI1 = 0;
 SBUF1 = c;
}

void putchar_s1 (char c) {
  if (c=='\n') _transchar_s1('\r'); // transmit \r\n
  _transchar_s1(c);  
  if (c == '\r' ) _transchar_s1('\n'); // transmit \r\n
}
