...
# Provided targets:
#
# default target: creates $(BASENAME).hex
...
.PHONY: hex iic bix load clean clean-all
...
$(BUILDDIR)/$(BASENAME).iic: $(BUILDDIR)/$(BASENAME).hex
	$(FX2LIBDIR)/utils/hex2iic.py -v $(VID) -p $(PID) $< $@

load: $(BUILDDIR)/$(BASENAME).bix
	fx2load -v $(VID) -p $(PID) $(BUILDDIR)/$(BASENAME).bix

clean:
	rm -f $(BUILDDIR)/*.{asm,hex,lnk,lst,map,mem,rel,rst,sym,adb,cdb,bix}
